...
| Implementation and tests | The adapter raised `NotImplementedError`. | Implemented and tested in public: [c1778e6](https://github.com/NiyaNagi/washington-sds150-favorites/commit/c1778e607fe9e22f61bffd482f35458c8824a10c), with follow-up fixes in the commits after it on `main`. Still unable to send a request without a local enable flag (off by default) and the user's own `rbuapp_` token. |
