...
| DSTARInfo | `dstarinfo` | The D-STAR repeater directory (`apps.dstarinfo.com`): all 22 areas, plus each repeater's detail page in Washington, Oregon, Idaho and British Columbia | Weekly (details monthly) | Callsign, city, gateway directory, and every module's output and offset. A cached reference for the D-STAR lists; no list consumes it yet. Personal use only under DSTARInfo's terms, so the pages are never committed |
| DSTARInfo DR repeater list | `dstarinfo_fm` (opt-in: runs only when named) | The 2,500 FM repeaters nearest home, out to about 730 miles, from DSTARInfo's repeater-list download for DR-mode radios: call, place, output, duplex, offset, tone mode and tone, approximate position | Weekly, when asked | For the operator's personal use only: the FM rows are RepeaterBook's data, passed on by DSTARInfo. Kept in the ignored local cache, never committed, never run by `sources update` unless named (`sources fetch dstarinfo_fm`), and separate from the approval-gated RepeaterBook API adapter. No list consumes it |
| Amateur repeater registry | derived `HAMREG` (`wasds150.catalog.repeater_registry`) | Every amateur repeater the catalog holds, one record per machine, within 750 miles of home | On every catalog load | Merges WWARA, IACC, operator-published lists, `DSTARINFO`, Seattle ACS, RadioReference county lists, `DSTARFM` and the FTX-1 import. WWARA and IACC records decide which copies are one machine; fields by precedence in that order; positions only from WWARA, then DSTARInfo/RepeaterBook approximate positions; each record's note lists its sources. Departments are `<State> - <band or mode>`: the radios' ham blocks, the SDS150's HAM Repeaters list and Near Me pick from the Washington ones. Rebuilt locally from the ignored caches, never committed |
...
137-row catalog's public fields and content hash exactly. The original 78
statewide/core rows still match the checked-in CSV, while 59 public extension
