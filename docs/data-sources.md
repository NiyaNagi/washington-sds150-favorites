...
| DSTARInfo | `dstarinfo` | The D-STAR repeater directory (`apps.dstarinfo.com`): all 22 areas, plus each repeater's detail page in Washington, Oregon, Idaho and British Columbia | Weekly (details monthly) | Callsign, city, gateway directory, and every module's output and offset. A cached reference for the D-STAR lists; no list consumes it yet. Personal use only under DSTARInfo's terms, so the pages are never committed. Its download tools are not used: the DR repeater list mixes in RepeaterBook's FM data, which comes only through the RepeaterBook API |
