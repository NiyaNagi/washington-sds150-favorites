...
def test_every_baseline_generated_hpe_passes_full_semantic_and_parity_validation(tmp_path):
    ...
    assert len(result.enabled_favorites) == 143
    assert len(export.files) == 79
    ...
    for filename, data in export.files.items():
        ...
        # reference list such as the band plan is projected onto SDS150
        # capabilities first, so comparing against the catalog entry would
        # report every HF channel as missing.
        favorite = export.projected.get(filename) or by_key[filename.removesuffix(".hpe")]
...
def test_bundle_manifest_hashes_and_all_embedded_hpes_validate(tmp_path):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        assert len([name for name in declared if name.endswith(".hpe")]) == 79
