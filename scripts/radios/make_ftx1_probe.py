...
#: The FTX-1 has no banks. The programmer's memory grid offers no Bank column
#: and no Settings > Bank Settings view - the bank strings in FTX1_V5.dll are
#: inherited framework overrides, not this radio's. What it has instead is
#: **M-Grp**, one checkbox per memory, and Set Menu `55: MEM Group` on the
#: radio: a single subset of the memories, which is the shape of a Near Me
#: list and the only grouping this radio can express.
#:
#: Where the file stores it is unknown. Byte 0x00 is 0x01 on every in-use
#: record of every file seen so far, including all 553 memories read from the
#: radio, so the guess that its bit 1 is M-Grp has never been tested. Ticking
#: alternate rows is enough: whichever byte holds it will differ between
#: neighbours that are otherwise identical.
MGRP_ROWS = [
    ("MGRP-OFF-1", "leave M-Grp unticked"),
    ("MGRP-ON-1", "tick M-Grp"),
    ("MGRP-OFF-2", "leave M-Grp unticked"),
    ("MGRP-ON-2", "tick M-Grp"),
    ("MGRP-OFF-3", "leave M-Grp unticked"),
    ("MGRP-ON-3", "tick M-Grp"),
]
...
def main(argv=None) -> int:
    ...
    mgrp = build(
        [name for name, _ in MGRP_ROWS],
        [note for _, note in MGRP_ROWS],
        template,
    )
    mgrp_path = out / "ftx1-mgrp.FTX1"
    mgrp.save(mgrp_path)
    # M-Grp may not live in the record at all. A reference copy saved by the
    # programmer with nothing ticked lets the whole file be diffed, not only
    # the records; see decode_ftx1_probe.py --against.
    mgrp_ref = out / "ftx1-mgrp-reference.FTX1"
    mgrp.save(mgrp_ref)
    ...
    print(f"wrote {mgrp_path}   ({len(MGRP_ROWS)} memories)")
    print(f"wrote {mgrp_ref}    (open and re-save this one unedited)")
    print()
    ...
    print()
    print("The M-Grp probe needs both files: open the reference, save it with")
    print("nothing changed, then open ftx1-mgrp.FTX1, tick M-Grp on the rows")
    print("named MGRP-ON-*, and save that too.")
    return 0
