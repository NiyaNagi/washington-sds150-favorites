...
| `survey.py` | Multi-pass survey. `python survey.py p2` runs a phase; see `PHASES` for which antenna goes on which port. Every pass is kept so continuous and intermittent signals can be told apart. Fills the few points the firmware omits (e.g. 344.65 MHz in LOW mode) from neighbours and logs them |
...
| `hfnoise.py` | Per-band external noise Fa vs ITU-R P.372 categories |
